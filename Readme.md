- Skill 的撰写指南：研读anthropic 的Skill仓库得到的结论
  ...
  **正例**：
  ...
  | 模板类型 | 适用场景                                                     | 核心特点                                                     |
  | -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | 最小版   | 1. 先快速验证一个 Skill 是否值得存在 2. 任务边界清楚，流程较短 3. 暂时不需要额外资源、脚本和模板 | 1. 只有一个 `SKILL.md` 2. 强调先把 `description` 和主流程写对 3. 成本最低，最适合起步 |
  | 标准版   | 1. Skill 已经进入稳定复用阶段 2. 有一定背景资料、模板或重复性逻辑 3. 需要把正文和外部资源分层 | 1. 包含 `references/`、`scripts/`、`assets/` 2. 适合大多数业务 Skill 3. 在可维护性和复杂度之间比较平衡 |
  | 最大版   | 1. Skill 已经成为一个持续演进的能力包 2. 需要评估、测试、角色拆分或多领域扩展 3. 需要面向后续迭代和多人协作 | 1. 在标准版基础上增加 `agents/`、`evals/`、`docs/` 2. 适合复杂 Skill 的长期维护 3. 不建议一开始就默认使用该结构 |
